...
def make_feature_matrix(which_process, which_data, r=50, d=20, cthres=500, fthres = 10, n=500):
    ...
    if which_process == 2 | which_process == 3:
        ...
        review_counts = ddat.groupby('critic').count().sort_values('title', ascending=False)
        ...
        common_sorted = common_film_df.sort_values('ncommon_films', ascending=False).copy()
    ...
    if which_process == 2:
        ...
        sorted_number_reviews = number_reviews_film.sort_values(inplace=False)
    ...
    if which_process == 3:
        ...
        cor_df = pd.DataFrame(critic_corr, columns=['critic', 'corr'])
        
        #From r features, combine with correlation to cinemascore
        common_corr = pd.merge(common_sorted.head(r), cor_df, on='critic')
        common_corr_sorted = common_corr.sort_values('corr', ascending=False).copy()
...
def make_train_test(cdat, res_col='collapsed_cinemascore', pred_cols=['A.A Dowd']):
    ...
    itrain, itest = train_test_split(range(cdat.shape[0]), train_size=0.8, random_state=0)
...
def add_short_genre(crot):
    ...
    release_df = pd.DataFrame(list(zip(release_dict.keys(), release_dict.values())), columns=['title', 'idate'])
    release_date = []
...
def run_dynamic_prediction_xg(pred_df, crot_date, train_df, Ytrain, meta_cols,  critic_name_list, max_iter=10, mod_thres=100):
    ...
    critic_sort = pd.DataFrame(crot_critics.groupby('critic').count().sort_values('title', ascending=False)['title'])
    ...
    corr_sort = corr_df.sort_values('abs_cor', ascending=False)
    ...
    which_critics = merge_count_above.sort_values('score', ascending=False).ii.tolist()[0:int(max_iter)]
...
def rot_to_df(rot_url):
    ...
    for iletter in list(string.ascii_lowercase):
        try:
            ...
            print('cant read letter json:', iletter)

    #Make pd dataframe of reviews
    rot_df = pd.DataFrame(list(zip(critic_series, score_series, date_series, movie_names, tmeter)), \
